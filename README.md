...
--- `void appendLogFile(...);`
 - Appends given information in a specific format to the output log file. (*see "Data Files", and "Log Messages"*)
...
--- `void edit_inventoryData(...);`
 ...
 - Allows the user (granted they have the right privilege-level assignment value- (*see "Privilege Levels"*))
 - Used to change information about each `InventoryData{}` item (*see "Data Files" for the list of information that may be changed*). 
...
| WARN          | W6002    | UNAUTH_PRIV   | USER      | User with priv *privlev* attempted to change SKU *SKU*(...) | user unsucessfully tried to change an item *SKU* number without correct privilege level       |
 ...
 - All times generated by local system date/time. 
 - If a log file is not present, a new one will **not** be created.
